...
def render(width, height, rum, kamera, lights): #skicka ut fler strålar per pixel och kör average
    ...
    for j in range(width):
        ...
        for i in range(height):
            ...
            bild[i][j] = ray_trace(ray, rum, lights, kamera, i, j)
...
def ray_trace(ray, rum, lights, kamera, i, j):
    ...
    for antal in range(len(lights)): #för flera ljuskällor
        light = lights[antal]
...
lights = [Light(Point(1.5, -0.5, -10.0)), Light(Point(-5, -10, -2))]
...
with open("bild31.ppm", "w") as bild_fil:
    ppm(bild_fil, bild)
