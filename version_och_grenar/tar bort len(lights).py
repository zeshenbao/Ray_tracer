    ...
    for light in lights:
        ...
        for el in range(len(rum)):
            ...
            for k in lista:
                ...
                for n in range(i-area,i+area):
                    for m in range(j-area, j+area):
                        if rum[el].light_dots.get((str(n),str(m))) == 0 or rum[el].light_dots.get((str(n),str(m))) == 1:
...
def ray_trace(ray, rum, light, kamera, i, j):
    ...
    color += max(0, normal.dot(hit_light.direction))*material.diffuse*material.color * (light.point - hit_pos).length() * (1/15) #ej för negativa värden, vinklar större än 90 grader
    
    half_vec = (hit_cam + hit_light.direction).norm()
    #Lägger till specularljus, med Blinn-phong modellen
    color +=  (max(0, normal.dot(half_vec))**k)*material.specular*light.color * (light.point - hit_pos).length() * (1/15)
...
lights = [Light(Point(-2, -5, -2)),Light(Point(1.5, -0.5, -10.0)),]
...
with open("bild42.ppm", "w") as bild_fil:
    ppm(bild_fil, bild)
