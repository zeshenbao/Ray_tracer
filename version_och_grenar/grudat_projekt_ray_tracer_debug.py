    ...
    for light in lights:
        ...
        for el in range(len(rum)):
            ...
            for k in lista:
                ...
                if bild_ny.length()/bild[i][j].length() < 1.01:
                    bild[i][j] = bild_ny
                    
                else:
                    temp_bild = bild_ny
                    while bild_ny.length()/bild[i][j].length() > 1.01:
                        #print(bild_ny.length()/bild[i][j].length())
                    
                        bild_ny -= temp_bild * 0.001
...
def ray_trace(ray, rum, light, kamera, i, j, depth):
    ...
    if block_t != None:
        #print(block_t)
        #block_point = shadow_ray.point + shadow_ray.direction * min_bt
        block_point = hit_pos + shadow_ray.direction * min_bt

        if (light.point-block_point).length() < (light.point-hit_pos).length():
            #print(block_point.length())
            if int(block_point.length()) != int(hit_pos.length()):
                
                #print(block_point, hit_pos)

                obj_hit.shadow_dots[str(i),str(j)] = 1
                #return color #RGB(255,255,255) * -1 #RGB(0.1,0.1,0.1) * -0.2 * (1/(light.point - block_point).length()**(1/2))
                return RGB(0.1,0.1,0.1) * -2.9* (1/(light.point - block_point).length()**(1/2))
