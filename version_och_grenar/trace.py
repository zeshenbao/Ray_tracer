...
class Material():
    ...
    def __init__(self, color = RGB(255, 0, 0), ambient=0.0, diffuse=1.0, specular=1.0, reflection=0.5): #126, 157, 155
    ...
    for light in lights:
                        ...
                        if rum[el].light_dots.get((str(n),str(m))) == 0:
                            if bild[n][m].length() != 0:
                                if start_styrka != 0:
                                    ...
                                    if bild[i][j].length()/start_styrka < 1.05:
                                        bild[i][j] += bild[n][m] * (1/(bild[n][m].length())**(0.75)) * 0.05 *(1/(count)**(0.4)) # count justerar förhållande mellan shadow_dots/totala_dots, length justerar efter max styrkan
                                else:
                                    bild[i][j] += bild[n][m] * (1/(bild[n][m].length())**(0.75)) * 0.05 *(1/(count)**(0.4))
                #print(x,y)
        """
        ...
        for el in range(len(rum)):
            ...
            for k in lista:
                ...
                #print(bild_ny.length()/bild[i][j].length())
                if bild_ny.length()/bild[i][j].length() < 5:
                    bild[i][j] = bild_ny
                    
                else:
                    temp_bild = bild_ny
                    while bild_ny.length()/bild[i][j].length() > 5:
                        #print(bild_ny.length()/bild[i][j].length())
                    
                        bild_ny -= temp_bild * 0.05
...
def ray_trace(ray, rum, light, kamera, i, j, depth):
    ...
    """
    light = light
    shadow_ray = Ray((light.point - hit_pos*1.0001).norm(), hit_pos*1.0001) # multiplicerar med 1.0001 för att skilja från sig själv
    block_t = obj_hit.intersect(shadow_ray)

    if block_t != None:

        block_point = shadow_ray.point + shadow_ray.direction * block_t

        if (light.point-block_point).length() < (light.point-hit_pos*1.0001).length():
            #print(block_point, hit_pos)
            
            obj_hit.shadow_dots[str(i),str(j)] = 1
            return RGB(0.1,0.1,0.1) * -0.2 * (1/(light.point - block_point).length()**(1/2))
...
rum = [Sphere(0.6, Vector(0.75, -0.1, 1), Material(RGB(0,100,0))), Sphere(0.4, Vector(-0.75, -0.1, 2.25), Material()), Sphere(10000, Vector(0, 10000.5, 1), Chessboard(RGB(255, 255, 255), RGB(0, 0, 0), 0, 1.0, 1.0, 0.2))]

lights = [Light(Point(1.5, -0.5, -5)), Light(Point(-0.5, -5.5, 0.0))]
...
with open("bild67.ppm", "w") as bild_fil:
    ppm(bild_fil, bild)
