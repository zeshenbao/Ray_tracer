    ...
    for light in lights:
        ...
        for el in range(len(rum)):
            ...
            for k in lista:
                ...
                if bild_ny.length()/bild[i][j].length() < 2:
                    bild[i][j] = bild_ny
                    
                else:
                    temp_bild = bild_ny
                    while bild_ny.length()/bild[i][j].length() > 2:
                        #print(bild_ny.length()/bild[i][j].length())
    ...
    if block_t != None:
        ...
        if (light.point-block_point).length() < (light.point-hit_pos).length():
            ...
            if int(block_point.length()) != int(hit_pos.length()):
                ...
                return RGB(0.1,0.1,0.1) * -0.9 * (1/(light.point - block_point).length()**(1/2))
...
with open("bild77.ppm", "w") as bild_fil:
    ppm(bild_fil, bild)
