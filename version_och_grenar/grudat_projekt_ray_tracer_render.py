...
def ray_trace(ray, rum, lights):
    ...
    """
    for i in range(len(lights)): #för flera ljuskällor
        light = lights[i]
...
def main():
    ...
    kamera = Vector(0, 0, -1)
    rum = [Sphere(0.3, Vector(0,0,0), Material()), Sphere(0.05, Vector(0,1,0), Material())]
    ...
    with open("bild8.ppm", "w") as bild_fil:
        ppm(bild_fil, bild)
...
rum = [Sphere(0.1, Vector(0, -0.5,0), Material()),Sphere(0.3, Vector(0,0,0), Material())]
lights = [Light(Point(0, -2, 0))]
...
with open("bild17.ppm", "w") as bild_fil:
    ppm(bild_fil, bild)
