        ...
        for el in range(len(rum)):
            ...
            for k in lista:
                ...
                for n in range(i-area,i+area):
                    for m in range(j-area, j+area):
                        if rum[el].light_dots.get((str(n),str(m))) == 0:
                            if start_styrka != 0: #0 div
                                #print(bild[i][j].length()/start_styrka)
                                if bild[i][j].length()/start_styrka < 1000:
                                    bild[i][j] += bild[n][m] * (1/(bild[n][m].length())**(0.75)) * 0.015 *(1/(count)**(0.4)) # count justerar förhållande mellan shadow_dots/totala_dots, length justerar efter max styrkan
...
with open("bild37.ppm", "w") as bild_fil:
    ppm(bild_fil, bild)
