    ...
    for light in lights:
        ...
        for j in range(width):
            ...
            for i in range(height):
                ...
                if bild[i][j] == 0:
                    bild[i][j] = ray_trace(ray, rum, light, kamera, i, j)
                else:
                    bild[i][j] += ray_trace(ray, rum, light, kamera, i, j)
...
def ray_trace(ray, rum, light, kamera, i, j):
    ...
    ambient_val = material.ambient * RGB(0,0,0)
    ...
    color += max(0, normal.dot(hit_light.direction))*material.diffuse*material.color * (1/(light.point - hit_pos).length()) *4 #ej för negativa värden, vinklar större än 90 grader
...
lights = [Light(Point(-2, -5, -2)), Light(Point(1.5, -0.5, -15.0))]
...
with open("bild64.ppm", "w") as bild_fil:
    ppm(bild_fil, bild)
