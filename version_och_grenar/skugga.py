    ...
    for light in lights:
        ...
        for el in range(len(rum)):
            ...
            for k in lista:
                ...
                if bild[i][j].length() == 0:
                    bild[i][j] = bild_ny
                    
                
                elif bild_ny.length()/bild[i][j].length() < 2:
                    bild[i][j] = bild_ny
...
def ray_trace(ray, rum, light, kamera, i, j, depth):
    ...
    color += ambient_val * justera #lägger till ambient color
    color += max(0, normal.dot(hit_light.direction))*material.diffuse*material.get_color(hit_pos) * (1/(light.point - hit_pos).length()) *4* justera  #ej för negativa värden, vinklar större än 90 grader
    color +=  (max(0, normal.dot(half_vec))**k)*material.specular*light.color * (1/(light.point - hit_pos).length()) *4* justera 
...
rum = [Sphere(0.6, Vector(1.25, -0.1, 1), Material(RGB(0,100,0))),
       Sphere(0.4, Vector(-0.25, -0.5, 1.5), Material()),
...
with open("bild98.ppm", "w") as bild_fil:
    ppm(bild_fil, bild)
