...
def ray_trace(ray, rum, lights):
    ...
    for i in range(len(lights)): #för flera ljuskällor
        ...
        if block_t != None:
            ...
            if (light.point-block_point).dot(light.point-block_point) < (light.point-hit_pos).dot(light.point-hit_pos):
                #print(block_point)
                return color
...
def main():
    ...
    lights = [Light(Point(0, 5, 0))]
    

    bild = render(width, height, rum, kamera, lights)
 

    with open("bild7.ppm", "w") as bild_fil:
        ppm(bild_fil, bild)
